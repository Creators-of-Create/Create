...
public class BuilderTransformers {
	...
	public static <B extends CopycatBlock, P> NonNullUnaryOperator<BlockBuilder<B, P>> copycat() {
		return b -> b.initialProperties(SharedProperties::softMetal)
			...
			.addLayer(() -> com.simibubi.create.foundation.render.LegacyRenderTypes::solid)
			...
			.addLayer(() -> com.simibubi.create.foundation.render.LegacyRenderTypes::translucent)
	...
	public static <B extends TrapDoorBlock, P> NonNullUnaryOperator<BlockBuilder<B, P>> trapdoor(boolean orientable) {
		return b -> b.blockstate(() -> (c, p) -> {
				// TODO 26.2: restore orientable trapdoor blockstate generation against the new Registrate model API.
	...
	public static <B extends SlidingDoorBlock, P> NonNullUnaryOperator<BlockBuilder<B, P>> slidingDoor(String type) {
		return b -> b.initialProperties(() -> Blocks.IRON_DOOR)
			.properties(p -> p.requiresCorrectToolForDrops()
				.strength(3.0F, 6.0F))
			.blockstate(() -> (c, p) -> {
				// TODO 26.2: restore sliding door blockstate generation against the new Registrate model API.
				p.simpleBlock(c.get(), AssetLookup.partialBaseModel(c, p));
			})
			.addLayer(() -> com.simibubi.create.foundation.render.LegacyRenderTypes::cutoutMipped)
			.transform(pickaxeOnly())
	...
	private static <B extends EncasedCogwheelBlock, P> BlockBuilder<B, P> encasedCogwheelBase(BlockBuilder<B, P> b,
		...
		return encasedBase(b, drop).addLayer(() -> com.simibubi.create.foundation.render.LegacyRenderTypes::cutoutMipped)
			.onRegister(CreateRegistrate.casingConnectivity((block, cc) -> cc.make(block, casingShift.get(),
			...
			.blockstate(() -> (c, p) -> {
				...
				axisBlock(c, p, blockState -> {
					...
					return models.computeIfAbsent(suffix, key -> p.models()
						.withExistingParent(c.getName() + key, p.modLoc("block/" + blockFolder + "/block" + key))
				...
				p.getVariantBuilder(c.get())
					.forAllStatesExcept(s -> {
						...
						return ConfiguredModel.builder()
							.modelFile(models.computeIfAbsent(suffix, key -> p.models()
								.withExistingParent(c.getName() + key, p.modLoc("block/scaffold/block" + key))
		...
		return b -> b.initialProperties(SharedProperties::stone)
			...
			.blockstate(() -> (c, p) -> {
				...
				p.getVariantBuilder(c.get())
					...
					String modelName = prefix + "/" + shapeName;
					return ConfiguredModel.builder()
						.modelFile(models.computeIfAbsent(modelName, $ -> p.models()
							.withExistingParent(modelName, p.modLoc("block/belt_tunnel/" + shapeName))
	...
	public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> bell() {
		return b -> b.initialProperties(SharedProperties::softMetal)
			...
			.blockstate(() -> (c, p) -> {
				Map<String, ModelFile> models = new HashMap<>();
				p.horizontalBlock(c.getEntry(), state -> {
					String variant = state.getValue(BlockStateProperties.BELL_ATTACHMENT)
						.getSerializedName();
					return models.computeIfAbsent(variant, key -> p.models()
						.withExistingParent(c.getName() + "_" + key, p.modLoc("block/bell_base/block_" + key)));
				});
			})
			.item()
			.model(() -> (c, p) -> p.withExistingParent(c.getName(), p.modLoc("block/" + c.getName())))
	...
	public static ItemBuilder<PackageItem, CreateRegistrate> packageItem(PackageStyle style) {
		...
		return Create.registrate().item(style.getItemId()
			...
			.model(() -> (c, p) -> {
				...
				if (style.rare())
					model = p.withExistingParent(c.getName(), p.modLoc("item/package/custom" + size))
						.texture("2", p.modLoc("item/package/" + style.type()));
				else
					model = p.withExistingParent(c.getName(), p.modLoc("item/package/" + style.type() + size));
		...
		return b -> {
			...
			return item.model(() -> (c, p) -> p.withExistingParent(name + "_table_cloth", p.modLoc("block/table_cloth/item"))
				...
				.recipe((c, p) -> ShapelessRecipeBuilder.shapeless(p.itemLookup(), RecipeCategory.MISC, c.get())
					...
					.save(p, Create.asResource("crafting/logistics/" + c.getName() + "_clear").toString()))
				.build();
		};
	}
