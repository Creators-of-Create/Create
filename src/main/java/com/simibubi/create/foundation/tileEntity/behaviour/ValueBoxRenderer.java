...
public class ValueBoxRenderer {
	...
	public static void renderItemIntoValueBox(ItemStack filter, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		...
		BakedModel modelWithOverrides = itemRenderer.getModel(filter, Minecraft.getInstance().level, null, 1); // PORT: scale?
		...
		itemRenderer.renderStatic(filter, TransformType.FIXED, light, overlay, ms, buffer, 1); // PORT: scale?
	}
	...
	private static float customZOffset(Item item) {
		...
		if (item instanceof BlockItem) {
			...
			if (block instanceof FenceBlock)
				return NUDGE;
			if (block.getTags().contains(BlockTags.BUTTONS))
				return NUDGE;
