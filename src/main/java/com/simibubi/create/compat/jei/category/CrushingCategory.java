...
//	}
	...
	public List<Widget> setupDisplay(CrushingDisplay display, Rectangle bounds) {
		...
		for (int outputIndex = 0; outputIndex < results.size(); outputIndex++) {
			...
			widgets.add(Widgets.createDrawableWidget(((helper, matrixStack, mouseX, mouseY, delta) -> getRenderedSlot(display.getRecipe(), finalOutputIndex).render(matrixStack, /*getBackground().getWidth() / 2 + */offset + 19 * finalOutputIndex, 78))));
		}

		widgets.add(new AnimatedKineticsWidget(crushingWheels, 62, 59));
		return widgets;
	}
