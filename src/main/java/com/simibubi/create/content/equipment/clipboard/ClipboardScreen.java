...
public class ClipboardScreen extends AbstractSimiScreen {
	...
	protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < currentEntries.size(); i++) {
			...
			int iconY = y;
			clipboardEntry.icon.ifLeft(stack -> graphics.renderItem(stack, x + 54, iconY + 50))
				.ifRight(fluid -> GuiGameElement.of(fluid.getFluid())
					.<GuiGameElement.GuiRenderBuilder>at(x + 54, iconY + 66, 100)
					.scale(16)
					.render(graphics));
