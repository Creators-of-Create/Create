...
public class TrackTargetingBlockItem extends BlockItem {
	...
	public InteractionResult useOn(UseOnContext pContext) {
		...
		if (bezier) {
			...
			bezierNbt.put("Key", com.simibubi.create.foundation.utility.LegacyNbtUtilsBridge.writeBlockPos(bezierTrackPointLocation.curveTarget()
				.subtract(placedPos)));
			blockEntityData.put("Bezier", bezierNbt);
		}

		blockEntityData.put("TargetTrack", com.simibubi.create.foundation.utility.LegacyNbtUtilsBridge.writeBlockPos(selectedPos.subtract(placedPos)));

		LegacyBlockEntityDataComponentBridge.set(stack, ((IBE<?>) this.getBlock()).getBlockEntityType(), blockEntityData);
		stack.remove(AllDataComponents.TRACK_TARGETING_ITEM_SELECTED_POS);
		stack.remove(AllDataComponents.TRACK_TARGETING_ITEM_SELECTED_DIRECTION);
		stack.remove(AllDataComponents.TRACK_TARGETING_ITEM_BEZIER);
	...
	public boolean useOnCurve(BezierPointSelection selection, ItemStack stack) {
		...
		net.createmod.catnip.api.client.network.ClientNetworkHelper.INSTANCE.sendToServer(new CurvedTrackSelectionPacket(be.getBlockPos(), loc.curveTarget(),
