...
public record CustomTransformConfig(String block_type, String old_type, String new_type,
									ProgressionSoundConfig progression, CompletionSoundConfig completion,
									List<ProcessingParticleConfig> particles) {

	public record ProgressionSoundConfig(int interval, String id, float volume, float pitch, float volume_slope,
										 float pitch_slope) {
	...
	public record CompletionSoundConfig(String id, float volume, float pitch) {
	...
	public static final Codec<CustomTransformConfig> CODEC = RecordCodecBuilder.create(i -> i.group(
			...
			Codec.STRING.fieldOf("old_type").forGetter(e -> e.old_type),
			Codec.STRING.fieldOf("new_type").forGetter(e -> e.new_type),
