...
public class InWorldProcessing {
	...
	protected static HashMap<Block, BiFunction<IBlockReader, BlockPos, Type>> customTypeMap = new HashMap<>();
	protected static List<BiFunction<IBlockReader, BlockPos, Type>> customPredicateTypeHandlers = new ArrayList<>();

	public static void addTypeHandler(Block block, BiFunction<IBlockReader, BlockPos, Type> function) {
		customTypeMap.put(block, function);
	}

	public static void addCustomTypeHandler(BiFunction<IBlockReader, BlockPos, Type> function) {
		customPredicateTypeHandlers.add(function);
	}

	static {
		addTypeHandler(Blocks.CAMPFIRE, (block, pos) -> {
			//blockState.method_28500(CampfireBlock.LIT).orElse(false)
			BlockState state = block.getBlockState(pos);
			if(state.method_28500(CampfireBlock.LIT).orElse(false)) {
				return Type.SMOKING;
			}
			return null;
		});
	...
	public enum Type {
		...
		public static Type byBlock(IBlockReader reader, BlockPos pos) {
			...
			if(customTypeMap.containsKey(block)) {
				Type r = customTypeMap.get(block).apply(reader, pos);
				if(r != null) return r;
			}
			if (tags.contains(AllTags.AllBlockTags.FAN_SPLASHING.tag.getId()))
				return Type.SPLASHING;
			boolean ignoreHeatLevel = tags.contains(AllTags.AllBlockTags.FAN_IGNORE_HEAT_LEVEL.tag.getId());
			if (tags.contains(AllTags.AllBlockTags.FAN_SMOKING.tag.getId()) ||
					(!ignoreHeatLevel && getHeatLevelOf(blockState) == BlazeBurnerBlock.HeatLevel.SMOULDERING))
				return Type.SMOKING;
			if (tags.contains(AllTags.AllBlockTags.FAN_BLASTING.tag.getId()) ||
					(!ignoreHeatLevel && getHeatLevelOf(blockState).isAtLeast(BlazeBurnerBlock.HeatLevel.FADING)))
				return Type.BLASTING;
			for(BiFunction<IBlockReader, BlockPos, Type> handler : customPredicateTypeHandlers) {
				Type r = handler.apply(reader, pos);
				if(r != null) return r;
			}
			return Type.NONE;
		}
