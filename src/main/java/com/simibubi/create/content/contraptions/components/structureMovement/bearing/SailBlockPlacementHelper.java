...
public class SailBlockPlacementHelper {
	...
	public static void tick() {
		...
		Vector3d offsetVec = Vector3d.of(offset.getDirectionVec());
		...
		for (Direction caretDirection : Iterate.directions) {
			...
			Vector3d otherOffset = Vector3d.of(caretDirection.getDirectionVec()).scale(.25f);
			Vector3d start = offsetVec.scale(.75f)
				.add(otherOffset);
			Vector3d target = centerOf.add(offsetVec);
	...
	public static Direction getPlacementOffset(World world, Direction sailDirection, BlockPos pos, Vector3d hit) {
		Direction argMin = null;
		float min = Float.MAX_VALUE;
		Vector3d diffFromCentre = hit.subtract(VecHelper.getCenterOf(pos));
		for (Direction side : Iterate.directions) {
			...
			if (!world.getBlockState(pos.offset(side))
				...
				.isReplaceable())
				continue;
			float distance = (float) Vector3d.of(side.getDirectionVec()).distanceTo(diffFromCentre);
			if (distance > min)
				continue;
			min = distance;
			argMin = side;
