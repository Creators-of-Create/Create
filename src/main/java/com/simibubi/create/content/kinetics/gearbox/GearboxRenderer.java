	...
	protected void renderSafe(GearboxBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		...
		for (Direction direction : Iterate.directions) {
			...
			if (be.getSpeed() != 0 && be.hasSource()) {
				...
				Direction sourceFacing = LegacyDirectionBridge.nearest(source.getX(), source.getY(), source.getZ(), Direction.NORTH);
			...
			shaft.renderInto(ms, buffer.getBuffer(com.simibubi.create.foundation.render.LegacyRenderTypes.solid()));
		}
	}
