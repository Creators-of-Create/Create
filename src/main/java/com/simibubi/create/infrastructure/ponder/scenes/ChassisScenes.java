...
public class ChassisScenes {
	...
	public static void linearGroup(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.overlay().showOutlineWithText(wrong1, 80)
	...
	public static void linearAttachement(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.overlay().showOutlineWithText(column3, 80)
		...
		scene.overlay().showCenteredScrollInput(chassisPos, Direction.NORTH, 50);
		scene.overlay().showControls(blockSurface, Pointing.UP, 50).whileCTRL()
			.rightClick()
			.withItem(AllItems.WRENCH.asStack());
	...
	public static void radial(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.overlay().showOutlineWithText(obj.getValue(), 60)
			.colored(PonderPalette.GREEN)
		...
		scene.overlay().showOutlineWithText(ignored, 80)
			.colored(PonderPalette.RED)
	...
	public static void superGlue(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.overlay().showControls(util.vector().topOf(2, 2, 3), Pointing.DOWN, 40).withItem(AllItems.SUPER_GLUE.asStack())
				.rightClick();
		scene.idle(6);
		...
		ElementLink<WorldSectionElement> cogs = scene.world().makeSectionIndependent(
				util.select().fromTo(4, 1, 2, 3, 1, 2)
