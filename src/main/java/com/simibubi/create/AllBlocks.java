		...
		REGISTRATE.block("schematicannon", SchematicannonBlock::new)
			...
			.loot((lt, block) -> {
				...
				lt.add(block, builder.withPool(LootPool.lootPool()
						...
						.add(LootItem.lootTableItem(AllBlocks.SCHEMATICANNON.get().asItem())
//								.apply(CopyNbtFunction.copyData(CopyNbtFunction.DataSource.BLOCK_ENTITY)
//										.copy("Options", "BlockEntityTag.Options")) // PORT: once again as said below, got no clue whats going on here
				)));
			})
			.item()
	...
	public static final BlockEntry<SailBlock> SAIL_FRAME = REGISTRATE.block("sail_frame", SailBlock::frame)
			.initialProperties(SharedProperties::wooden)
			.properties(BlockBehaviour.Properties::noOcclusion)
	...
	public static final BlockEntry<SailBlock> SAIL = REGISTRATE.block("white_sail", SailBlock::withCanvas)
			.initialProperties(SharedProperties::wooden)
			.properties(BlockBehaviour.Properties::noOcclusion)
			.blockstate(BlockStateGen.directionalBlockProvider(false))
			.tag(AllBlockTags.WINDMILL_SAILS.tag)
	...
	public static final DyedBlockList<SailBlock> DYED_SAILS = new DyedBlockList<>(colour -> {
		...
		String colourName = colour.getSerializedName();
		return REGISTRATE.block(colourName + "_sail", SailBlock::withCanvas)
				.properties(BlockBehaviour.Properties::noOcclusion)
				.initialProperties(SharedProperties::wooden)
	...
	public static final BlockEntry<CopperBacktankBlock> COPPER_BACKTANK =
		REGISTRATE.block("copper_backtank", CopperBacktankBlock::new)
			...
			.loot((lt, block) -> {
				Builder builder = LootTable.lootTable();
				LootItemCondition.Builder survivesExplosion = ExplosionCondition.survivesExplosion(); // PORT: may be bad
				lt.add(block, builder.withPool(LootPool.lootPool()
					.when(survivesExplosion)
					.setRolls(ConstantValue.exactly(1))
...
//							.copy("Air", "Air"))
//						.apply(CopyNbtFunction.copyData(CopyNbtFunction.DataSource.BLOCK_ENTITY)
//							.copy("Enchantments", "Enchantments")) // PORT: what even does this do
						)));
			})
			.register();
